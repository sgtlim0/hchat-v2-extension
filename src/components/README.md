...
사이드패널의 각 탭과 공용 UI 요소를 구성하는 64개 React 컴포넌트 (.tsx 파일). 채팅 인터페이스, 크로스 모델 그룹 채팅, 크로스 모델 토론, 도구 뷰, 설정, 히스토리, 북마크, 프롬프트 라이브러리, 사용량 통계, 비서 마켓플레이스 등 모든 사용자 인터페이스를 포함한다.

**v6.0 (2026-03-07)**: 5개 프로바이더 (Ollama/OpenRouter 추가), MCP 서버 관리, 팀 공유, 워크플로우 편집기, AI 메모리 패널, 대화 분석/트리 시각화 추가. react-markdown 제거 (v5.1), PROVIDER_COLORS 중앙화. 총 59,974줄 중 컴포넌트 ~15,000줄.

## 파일 목록

| 파일 | 줄 수 | 설명 |
|------|-------|------|
| `ChatView.tsx` | 568 | 메인 채팅 인터페이스 — 메시지, 스트리밍, 에이전트, TTS/STT, 내보내기, 분기, 딥리서치 |
