...
현재 문서 번역은 TXT/CSV/XLSX만 지원. PPTX/PDF까지 확장하고 문서 관리 기능을 추가한다.

| 항목 | 설명 | 난이도 |
|------|------|--------|
| PPTX 번역 | pptxgenjs로 ZIP 해제 → XML 텍스트 추출 → 번역 → 재압축 | 높음 |
| PDF 텍스트 번역 | pdfjs-dist (이미 사용 중)로 텍스트 추출 → 번역 → 텍스트 PDF 재생성 | 중간 |
| 문서 프로젝트 관리 | Storage CRUD, 프로젝트별 문서 히스토리, 버전 관리 | 중간 |
| 양식 기반 작성 | DOCX 템플릿 업로드 → AI 구조 분석 → 빈칸 채우기 | 높음 |

**새 의존성**: `pptxgenjs` (~50KB, 동적 임포트), `mammoth` (~16KB, 동적 임포트)
