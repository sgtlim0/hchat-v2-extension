...
npm test         # Run all tests (Vitest, 649 tests, 36 files)
...
3 locales: Korean (primary), English, Japanese. Lightweight custom implementation — `t()` function + `useLocale()` hook. 670+ keys per locale. Content scripts use `tSync()` + `getLocale()`.
...
- Tests: Vitest with chrome.storage.local mock, 649 tests across 36 files
