...
- 관련도 랭킹: BM25 × 0.7 + 최신성 × 0.3 (v5.3 BM25 스코어링 + IDF 도입)
...
### 23. 다국어 지원 (i18n)
- 한국어/영어/일본어 3개 언어 지원
- 경량 자체 구현 (외부 라이브러리 미사용, 931+ 키/언어)
...
### 33. 음성 대화 E2E 파이프라인 (v5.5 신규)
- **STT → AI → TTS 자동 루프**: 음성 입력 → AI 응답 → 음성 출력 연속 파이프라인
- 침묵 감지 자동 전송
- 포즈/리줌 제어

### 34. 비서 체인 (v5.5 신규)
- 비서1 → 비서2 → 비서3 순차 파이프라인 실행
- `{{input}}` / `{{original}}` 변수 치환 (이전 비서 출력/원본 입력)
- 체인 내보내기/가져오기 (JSON)

### 35. 토론 투표 시스템 (v5.5 신규)
- 토론 참가자별 1~5점 투표
- 스코어보드 랭킹 시각화
...
- **비서 추천**: 감지된 의도에 최적화된 내장/커스텀 비서 자동 추천
- **도구 추천**: 의도에 맞는 AI 도구 자동 추천
- **사용 패턴 학습**: 가중 빈도 추적 + 시간 감쇠로 사용자 선호도 학습, top 3 추천
- **컨텍스트 자동 요약**: 20+ 메시지 대화 자동 요약, FIFO 캐시, 시스템 프롬프트 주입
- **BM25 검색 개선**: BM25 스코어링 + IDF 도입, 검색 정확도 향상 (BM25 70% + 최신성 30%)
...
| PDF | pdfjs-dist | 4.x |
| i18n | 자체 구현 (t, useLocale, tSync) | 931+ keys |
...
└── src/
    ...
    │   ├── en.ts                  # 영어 번역 (931+ 키)
    │   └── ja.ts                  # 일본어 번역 (931+ 키)
    ├── components/                 # 65개 컴포넌트 (ChatInputArea, AssistantSelector, ModelSelector 수정)
    ...
    │   ├── intentRouter.ts        # AI 자동 추천 — 9가지 의도 감지, 비서/도구 추천 (v5.3)
    │   ├── userPreferences.ts     # 사용 패턴 학습 — 가중 빈도 추적, 시간 감쇠, top 3 (v5.3)
    │   ├── conversationSummarizer.ts # 컨텍스트 자동 요약 — 20+ 메시지, FIFO 캐시 (v5.3)
    │   ├── bm25.ts                # BM25 검색 — IDF, combinedScore (70%+30%) (v5.3)
    │   ├── voicePipeline.ts      # 음성 대화 E2E 파이프라인 — STT→AI→TTS 루프, 침묵 감지 (v5.5)
...
| 최종 통계 | 55개 컴포넌트, 65개 lib 파일, 1787 tests (93 files), 80%+ branch coverage, ESLint 0 errors, 835+ i18n 키 |
...
### v6.0.0 (2026-03-07) — 플랫폼 확장 & 엔터프라이즈

| 기능 | 설명 |
|------|------|
| Ollama 프로바이더 | ollama-provider.ts — 로컬 LLM (NDJSON 스트리밍, 동적 모델 탐색) |
| OpenRouter 프로바이더 | openrouter-provider.ts — 100+ 모델 (OpenAI SSE 호환, 5 프리셋) |
| 프로바이더 통합 | types.ts ProviderType 확장, provider-factory.ts 자동 생성 |
| 팀 공유 | teamSharing.ts + TeamSharingPanel.tsx — 패키지 생성/검증/적용, 히스토리 |
| MCP 클라이언트 | mcpClient.ts + McpServerManager.tsx — 서버 등록, 도구 실행, 리소스 접근 |
| 감사 로그 | auditLog.ts — 액션 기록, 필터/검색, CSV/JSON 내보내기, 90일 정리 |
| 정책 관리 | policyManager.ts — 5종 정책 (모델/도구 화이트리스트, PII, 예산, 승인) |
| Firefox 어댑터 | firefoxAdapter.ts — 브라우저 감지, manifest 변환, 사이드패널 어댑터 |
| 통계 | 65개 컴포넌트, 81개 lib 파일, 5개 프로바이더, 2232 tests (118 files), 931+ i18n 키 |

### v5.7.0 (2026-03-06) — 고급 기능 9개 모듈 + 6개 UI

| 기능 | 설명 |
|------|------|
| 컨텍스트 윈도우 최적화 | contextOptimizer.ts — 토큰 카운팅, 메시지 압축, 컨텍스트 모니터링 |
| 스마트 프롬프트 캐시 | promptCache.ts — BM25 유사도 감지, TTL 24h, FIFO 100 |
| 대화 분석 대시보드 | analyticsEngine.ts + ConversationAnalytics.tsx — TF-IDF 토픽, 히트맵 |
| AI 메모리 시스템 | aiMemory.ts + MemoryPanel.tsx — 자동 추출 (이름/선호/프로젝트), 승인/거부 |
| 대화 분기 트리 | conversationTree.ts + ConversationTreeView.tsx — 포크 트리 시각화, 병합 |
| 스마트 응답 템플릿 | responseTemplate.ts + ResponseStyleSelector.tsx — 4 프리셋, 사용 패턴 학습 |
| 멀티모달 입력 | multimodalInput.ts + MultimodalPreview.tsx — 다중 이미지, 스크린샷, 리사이즈 |
| 실시간 협업 | collaborationMode.ts + CollaborationBadge.tsx — BroadcastChannel 탭 동기화 |
| AI 워크플로우 빌더 | workflowBuilder.ts — 노드 기반 워크플로우, 조건 분기, 순환 감지 |
| 통계 | 61개 컴포넌트, 74개 lib 파일, 2055 tests (108 files), 893+ i18n 키 |

#### v5.6 Phase 3 — 비서 체인 + 음성 대화 통합 (2026-03-06)
| 항목 | 내용 |
|------|------|
| 비서 체인 UI | ChainBuilder.tsx 신규 (CRUD, 단계 추가/삭제, export/import) |
| 음성 대화 UI | VoiceConversation.tsx 신규 (상태 표시, 포즈/리줌, 대화 로그) |
| i18n 확장 | chain 12개 + voice 10개 = 22개 키 추가 (ko/en/ja) |
| 테스트 강화 | provider-factory 32 + useChat 20 + ChainBuilder 13 + VoiceConversation 12 = +77 tests |
| 최종 통계 | 55개 컴포넌트, 65개 lib 파일, 1787 tests (93 files), 835+ i18n 키 |

### v5.5.0 (2026-03-05) — 음성 파이프라인 & 비서 체인 & 토론 투표

| 기능 | 설명 |
|------|------|
| 음성 대화 E2E 파이프라인 | voicePipeline.ts — STT → AI → TTS 자동 루프, 침묵 감지, 포즈/리줌 |
| 비서 체인 | assistantChain.ts — 비서1→비서2→비서3 순차 파이프라인, {{input}}/{{original}} 치환, export/import |
| 토론 투표 시스템 | debateVoting.ts — 1~5점 투표, 스코어보드 랭킹, 컨센서스 도출 (threshold 0.7) |
| 키보드 단축키 확장 | shortcutManager.ts — 포커스 트랩, 키 레코더, 예약 콤보 검증, 플랫폼별 표시 |
| 통계 | 64개 lib 파일, 52개 컴포넌트, 1311 tests (63 files), 730+ i18n 키 (ko/en/ja) |

### v5.4.0 (2026-03-05) — 추천 UI 통합 & 검색 고도화

| 기능 | 설명 |
|------|------|
| ChatInputArea 의도 추천 칩 | detectIntent() 실시간 300ms 디바운스, 의도별 아이콘 칩, onApplyIntent 콜백 |
| useChat 사용 추적 + 자동 요약 | trackUsage() 모델/비서 추적, 20+ 메시지 자동 요약, 시스템 프롬프트 주입 |
| 비서/모델 추천 배지 | AssistantSelector ⭐ 추천 + 카테고리 탭, ModelSelector ⭐ 배지 |
| BM25 검색 통합 | messageSearch.ts calculateScore를 BM25 기반으로 교체, trigram 유지 |
| 통계 | 50개 lib 파일, 1210 tests (59 files), 730+ i18n 키 (ko/en/ja) |

### v5.3.0 (2026-03-05) — AI 자동 추천 & 검색 고도화

| 기능 | 설명 |
|------|------|
| AI 자동 추천 시스템 | intentRouter.ts — 9가지 의도 감지 (코드/번역/요약/분석/문서/이미지/검색/대화/기타), 비서/도구 자동 추천 |
| 사용 패턴 학습 | userPreferences.ts — 가중 빈도 추적 + 시간 감쇠, top 3 추천, `hchat:user-prefs` 저장 |
| 컨텍스트 자동 요약 | conversationSummarizer.ts — 20+ 메시지 대화 자동 요약, FIFO 캐시 (`hchat:conv-summaries`), 시스템 프롬프트 주입 |
| BM25 검색 개선 | bm25.ts — BM25 스코어링 + IDF 도입, combinedScore (BM25 70% + 최신성 30%) |
| 통계 | 50개 lib 파일, 1148 tests (54 files), 730+ i18n 키 (ko/en/ja) |

### v5.2.0 (2026-03-05) — 성능 & 접근성 강화

| 항목 | 내용 |
|------|------|
| 가상 스크롤 | react-window VariableSizeList (50+ 메시지), MsgBubble React.memo, MarkdownRenderer useMemo |
| 접근성 WCAG AA | CSS 대비 개선 (--text2/#8b9cb8, --text3/#5d7090), focus-visible, ARIA labels (ChatInputArea, ModelSelector, MsgBubble), 동적 lang 속성 |
| 테스트 확대 | 993 tests (50 files) — 인프라 146 + 에이전트/훅 106 신규 |
| 빌드 | sidepanel ~132KB (46KB gzip), 성공 |

### v5.1.0 (2026-03-05) — 추가 최적화 및 기술 부채 해소

| 항목 | 내용 |
|------|------|
| react-markdown 제거 | react-markdown, remark-gfm, rehype-highlight 의존성 제거 (96 패키지, ~130KB 절약) |
| PROVIDER_COLORS 통합 | 7개 컴포넌트 하드코딩 색상 → types.ts 중앙 관리 (Bedrock/OpenAI/Gemini) |
| toolbar.ts i18n 통합 | 하드코딩 프롬프트 → tSync() 함수, 일본어 지원 추가 (6개 키) |
| 통계 업데이트 | 726+ i18n 키 (ko/en/ja), 57개 컴포넌트, 56개 lib 파일 |
| 품질 | 741 tests (40 files), ESLint 0 errors |
...
#### v5.1 추가 최적화 (2026-03-05)
| 항목 | 내용 |
|------|------|
| 의존성 최적화 | react-markdown, remark-gfm, rehype-highlight 제거 → 96 패키지 절약, ~130KB 번들 감소 |
| 프로바이더 색상 통합 | 7개 컴포넌트 하드코딩 → PROVIDER_COLORS 상수 중앙 관리 (types.ts) |
| toolbar.ts i18n | 하드코딩 프롬프트 → tSync() 국제화, 일본어 지원 추가 (6개 키) |
| 최종 통계 | 726+ i18n 키 (ko/en/ja), 57개 컴포넌트, 56개 lib 파일 |
...
### 규모 비교
| 항목 | v1 | v2 | v3 | v3.6 | v4.2 | v4.3 | v4.5 | v5.0 | v5.1 | v5.2 | v5.3 | v5.4 | v5.5 | v5.6 | v5.7 | v6.0 |
|------|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|
| 소스 파일 | ~22개 | ~40개 | ~50개 | ~80개 | ~90개 | ~100개 | ~107개 | ~115개 | ~115개 | ~125개 | ~133개 | ~138개 | ~146개 | ~150개 | ~180개 | ~200개 |
| 코드 라인 | ~4,000 | ~8,000 | ~10,000+ | ~15,000+ | ~18,000+ | ~20,000+ | ~21,500+ | ~25,000+ | ~25,000+ | ~26,000+ | ~27,000+ | ~28,000+ | ~30,000+ | ~31,000+ | ~38,000+ | ~42,000+ |
| 탭 수 | 5개 | 7개 | 8개 | 8개 | 8개 | 8개 | 8개 | 8개 | 8개 | 8개 | 8개 | 8개 | 8개 | 8개 | 8개 | 8개 |
| 컴포넌트 (.tsx) | 12개 | 18개 | 24개 | 40개 | 48개 | 50개 | 52개 | 52개 | 52개 | 52개 | 52개 | 52개 | 52개 | 55개 | 61개 | 65개 |
| lib 파일 | 6개 | 20개 | 30개 | 45개 | 50개 | 55개 | 57개 | 56개 | 56개 | 56개 | 50개 | 50개 | 64개 | 65개 | 74개 | 81개 |
| AI 프로바이더 | 1개 | 1개 | 3개 | 3개 | 3개 | 3개 | 3개 | 3개 | 3개 | 3개 | 3개 | 3개 | 3개 | 3개 | 3개 | 5개 |
| 지원 모델 | 3개 | 3개 | 9개 | 9개 | 9개 | 9개 | 9개 | 9개 | 9개 | 9개 | 9개 | 9개 | 9개 | 9개 | 9개 | 14+개 |
| 도구 | 4개 | 8개 | 8개 | 12개 | 15개 | 16개 | 16개 | 17개 | 17개 | 17개 | 17개 | 17개 | 17개 | 17개 | 17개 | 17개 |
| 내장 비서 | 0개 | 0개 | 0개 | 0개 | 0개 | 0개 | 8개 | 20개 | 20개 | 20개 | 20개 | 20개 | 20개 | 20개 | 20개 | 20개 |
| 테스트 | 0개 | 0개 | 0개 | 365개 | 498개 (30 파일) | 589개 (34 파일) | 649개 (36 파일) | 741개 (40 파일) | 741개 (40 파일) | 993개 (50 파일) | 1148개 (54 파일) | 1210개 (59 파일) | 1311개 (63 파일) | 1787개 (93 파일) | 2055개 (108 파일) | 2232개 (118 파일) |
| i18n 키 | 0개 | 0개 | 0개 | 420+ (ko/en) | 600+ (ko/en/ja) | 650+ (ko/en/ja) | 670+ (ko/en/ja) | 720+ (ko/en/ja) | 726+ (ko/en/ja) | 726+ (ko/en/ja) | 730+ (ko/en/ja) | 730+ (ko/en/ja) | 730+ (ko/en/ja) | 835+ (ko/en/ja) | 893+ (ko/en/ja) | 931+ (ko/en/ja) |
