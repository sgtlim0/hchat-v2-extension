...
- **문서 번역**: TXT/CSV/XLSX/PPTX/PDF 포맷 유지 번역, 용어 일관성, 비용 추정
- **문서 작성**: AI 자동 문서 생성 (보고서/이메일/제안서/회의록/메모 → DOCX), 프로젝트 관리
- **템플릿 문서**: DOCX 템플릿 업로드 → {{필드}} 자동 추출 → AI 내용 생성
- **배치 OCR**: 10장 동시 업로드, 명함/영수증 구조화 추출
- **이미지 생성**: DALL-E 3 (3가지 크기, HD 품질, 세션 히스토리)
...
- **문서 프로젝트**: 작성한 문서 저장/관리, 버전 히스토리 (최대 10)
- **다국어**: 한국어, English, 日本語 (650+ keys)
...
### 스크린샷 목록 (v4.2 업데이트 필요)
1. **커스텀 비서** — 비서 선택 + 대화 화면
2. **문서 번역/작성** — 파일 업로드 + 번역/작성 진행
3. **이미지 생성** — DALL-E 3 프롬프트 + 생성된 이미지 갤러리
4. **데이터 분석** — CSV 업로드 + SVG 차트 시각화
