...
- **문서 번역**: TXT/CSV/XLSX/PPTX/PDF 포맷 유지 번역, 용어 일관성, 비용 추정, 중단 기능, 진행 상황 표시 (경과 시간 + 예상 남은 시간)
- **문서 작성**: AI 자동 문서 생성 (보고서/이메일/제안서/회의록/메모 → DOCX), 프로젝트 관리 (검색+필터)
- **템플릿 문서**: DOCX 템플릿 업로드 → {{필드}} 자동 추출 → AI 내용 생성, 갤러리 저장/재사용, 템플릿 공유 (내보내기/가져오기 JSON)
