...
- **짧은 설명** (132자 이내): AWS Bedrock, OpenAI, Gemini를 하나의 사이드바에서. 20개 AI 비서, 음성 대화, 비서 체인, PPT 기획, 문서 번역/작성, 딥 리서치.
- **카테고리**: Productivity
- **언어**: 한국어, English, 日本語
...
- **음성 대화 E2E**: 음성 입력 → AI 처리 → TTS 재생 자동 루프, 침묵 감지, 포즈/리줌 (Web Speech API)
- **비서 마켓플레이스**: 20개 내장 비서 (6카테고리), 검색/필터, 내보내기/가져오기
- **비서 체인**: 비서1→비서2→비서3 순차 파이프라인, {{input}}/{{original}} 변수 치환, 최대 20개 체인 저장
- **AI 토론 투표**: 1~5점 투표, 스코어보드 랭킹, 컨센서스 도출 (threshold 0.7)
- **키보드 단축키 확장**: 포커스 트랩, 키 레코더, 예약 콤보 검증, 플랫폼별 표시 (Mac⌘/⇧/⌥)
- **AI 가드레일**: 개인정보(PII) 자동 감지 → 마스킹 후 전송 (이메일/전화/주민번호/카드/계좌)
- **PPT 기획**: 주제 입력 → AI 슬라이드 구조 생성 → PPTX 다운로드
- **대화 템플릿**: 다단계 대화 흐름 저장/재사용, {{변수}} 지원
...
- **AI 토론**: 두 AI 모델 간 3라운드 자동 토론, 비서 vs 비서 토론
...
- **다국어**: 한국어, English, 日本語 (730+ i18n keys)
...
### 스크린샷 목록 (v5.5 업데이트 필요)
1. **음성 대화** — 음성 파이프라인 UI, 포즈/리줌 컨트롤, 실시간 STT/TTS
2. **비서 체인** — 체인 빌더 UI, 비서 순차 실행, 변수 치환
3. **AI 토론 투표** — 토론 뷰, 투표 UI (1~5점), 스코어보드
4. **문서 번역** — 파일 업로드 + 실시간 진행 상황 (ETA)
5. **템플릿 문서** — DOCX 템플릿 갤러리 + 필드 자동 생성
